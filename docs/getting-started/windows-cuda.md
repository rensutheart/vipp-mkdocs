...
VIPP 0.16.0a2 uses one standard CUDA 13 installation for every current
...
!!! warning "Use the exact 0.16.0a2 release"
    Download the installer and checksum only from the
    [official v0.16.0a2 release](https://github.com/rensutheart/napari-vipp/releases/tag/v0.16.0a2).
    Verify `VIPP-Setup-0.16.0a2-Windows-x86_64-UNSIGNED.exe` against
    `SHA256SUMS-Windows-0.16.0a2.txt` before opening it. This alpha is
...
**New in 0.15.0a5:** CUDA installations use one **VIPP** shortcut and
...
The completed-node badge reports what actually ran. GPU providers in 0.16.0a2
...
In 0.16.0a2, Prefer GPU preserves exact workload facts across intervening
