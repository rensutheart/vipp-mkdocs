...
Use this checklist when someone asks you to test the tagged VIPP `0.16.0a1`
...
!!! warning "Use the exact tagged installer"
    Use this checklist only with the exact `v0.16.0a1` GitHub asset and its
    checksum from the
    [official release](https://github.com/rensutheart/napari-vipp/releases/tag/v0.16.0a1).
...
Download the installer and checksum file only from the official `v0.16.0a1`
entry on the
[VIPP releases page](https://github.com/rensutheart/napari-vipp/releases).
The filename should be
`VIPP-Setup-0.16.0a1-Windows-x86_64-UNSIGNED.exe`. In PowerShell, run:
...
`SHA256SUMS-Windows-0.16.0a1.txt`. Stop and delete the file if they differ. This
...
version do not count as a pass for this downloaded `0.16.0a1` installer.
