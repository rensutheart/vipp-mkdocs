...
VIPP 0.16.0a2 provides separate offline packages for Apple Silicon and Intel
...
the [official v0.16.0a2 release](https://github.com/rensutheart/napari-vipp/releases/tag/v0.16.0a2):

- **Apple Silicon** — the Mac reports a chip such as Apple M1, M2, M3, or M4:
  download `VIPP-0.16.0a2-macOS-arm64-UNSIGNED.pkg` and
  `SHA256SUMS-macOS-arm64-0.16.0a2.txt`.
- **Intel** — the Mac reports an Intel processor: download
  `VIPP-0.16.0a2-macOS-x86_64-UNSIGNED.pkg` and
  `SHA256SUMS-macOS-x86_64-0.16.0a2.txt`.
...
=== "Apple Silicon"
    ...
    ```bash
    shasum -a 256 VIPP-0.16.0a2-macOS-arm64-UNSIGNED.pkg
    ```

=== "Intel"

    ```bash
    shasum -a 256 VIPP-0.16.0a2-macOS-x86_64-UNSIGNED.pkg
    ```
