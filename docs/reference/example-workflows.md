...
| --- | --- | --- |
| `exhaustive-inspector-showcase.json` | nine lanes using eight synthetic samples, including thresholding, label boundaries, Propagation and the CellProfiler compartment profile | Comprehensive manual review of every palette operation, connected-input summaries, scientific controls, and result sections. Both CellProfiler lanes are unreleased. The compartment lane exercises controls on synthetic guidance; use the [paper workflow](../workflows/statistics-paper-compartments.md) for the scientific wiring and comparison boundaries. |
